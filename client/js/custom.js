...
$(document).mouseup(function(e){
    hideelewhenoutsideclick('.emojiwrapper', e);
});


function hideelewhenoutsideclick(ele, e){
    var container = $(ele);
...
function replaceEmotions(html){
    // varloadingicons();
    var lowericon = html.toLowerCase();
    var search = '';
    var loadicons = loadingicons();
    for(var i in loadicons)
      	search += i.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&')+'|';//escape characters and add 'or'
    	search = search.substr(0, search.length-1);//remove last 'or'

    var replaceCallback = function(match) {
      return '<img src="img/emoji/' + loadicons[match] + '" />';
    }

    return lowericon.replace(new RegExp(search, 'gm'), replaceCallback);
}
